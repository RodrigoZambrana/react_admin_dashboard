...
const FALLBACK_CONFIG: StorefrontConfig = {
  ...
  policies: [
    { title: "Shipping & delivery", body: "We ship worldwide within 3-5 business days.", updatedAt: new Date().toISOString() },
    { title: "Returns", body: "Returns accepted within 30 days in original condition.", updatedAt: new Date().toISOString() },
  ],
  announcement: {
    id: "free-shipping",
...
export const resolveHomeLayout = async (layoutKey?: string): Promise<HomeLayoutDefinition> => {
  ...
  try {
    ...
    return DEFAULT_HOME_LAYOUTS.find((candidate) => candidate.key === FALLBACK_LAYOUT_KEY)! // fallback always exists
  }
}
