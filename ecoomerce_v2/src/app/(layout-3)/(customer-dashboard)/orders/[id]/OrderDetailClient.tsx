...
export default function OrderDetailClient({ identifier }: OrderDetailClientProps) {
  const { order, loading, error, refresh } = useAccountOrder(identifier);
  ...
  const orderReference = order.reference ?? order.orderNumber ?? String(order.id);
      ...
      <Card p="0px" mb="30px" overflow="hidden" borderRadius={12}>
        <TableRow bg="gray.200" p="12px" boxShadow="none" borderRadius={0}>
          <FlexBox className="pre" m="6px" alignItems="center">
            ...
            <Typography fontSize="14px">#{orderReference.substring(0, 8)}</Typography>
          </FlexBox>
      ...
      <Grid container spacing={6}>
        ...
        <Grid item lg={6} md={6} xs={12}>
          <Card p="20px 30px" borderRadius={12}>
            ...
            <Typography fontSize="14px">Payment status: {order.paymentStatus}</Typography>
          </Card>
        </Grid>
      </Grid>
    </>
