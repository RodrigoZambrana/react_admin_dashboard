# Ecoomerce V2 Integration Plan

This document outlines actionable steps required to replace the existing Wokiee-based storefront with the Bonik “ecommerce_v2” template while preserving the backend-ready architecture developed so far.

## Phase 1 – Audit & Prepare

- Install dependencies and run `npm run dev` in `ecoomerce_v2` to review the template out of the box.
...
- **State providers**: `src/contexts/CartContext.tsx` seeds cart state from `@data/cart` and handles quantity changes locally. We should retire this in favor of the persisted cart/auth flows that live in the existing `ecommerce/` project.
...
- **HTTP client & API bindings**: Replace Bonik's `src/lib/axios.ts` + mock endpoints with the existing `StorefrontApi`/`apiFetch` stack from `../ecommerce/src/lib/api/storefront.ts` and `../ecommerce/src/lib/http.ts`. These already enforce HTTPS, timeouts, error typing (`ApiError`), and environment-driven base URLs.
- **Configuration & layouts**: Bring over `getStorefrontConfig` and `resolveHomeLayout` from `../ecommerce/src/lib/storefront-config.ts` along with `DEFAULT_HOME_LAYOUTS` in `../ecommerce/src/lib/layouts/homeLayouts.ts`. They preserve the 12 home layout variants and navigation schema expected by the admin dashboard.
- **Global providers**: Swap Bonik's `CartContext` for the persisted cart provider in `../ecommerce/src/state/cart-context.tsx`. Plan a similar bridge for session handling (reuse the login/register flows built under `../ecommerce/src/app/(storefront)/account/*`).
...
- **Utility helpers**: Reuse formatting helpers (e.g. `normalizeMoney`) and shared types from `../ecommerce/src/lib/utils` and `../ecommerce/src/types/storefront.ts` to avoid drift between UI variants and backend contracts.
- **Legacy Wokiee UI**: Treat `../ecommerce/src/theme/**` and related contexts as reference only; Bonik components should replace them while consuming the mapped data sources above.

## Phase 2 – Create Migration Scaffold

- Keep `ecommerce/` as a reference (especially `lib/api/storefront.ts`, auth flows, and configuration modules).
- Inside `ecoomerce_v2/src`, create a dedicated namespace (`src/app/storefront` or similar) where migrated pages will be rewritten to use the Bonik structure but rely on our REST API client.
...
- When the ecoomerce_v2 tree is production-ready, rename it to `ecommerce/` and archive the old tree for reference.
...
| --- | --- | --- | --- |
| Data fetching | `src/lib/axios.ts`, `src/utils/__api__/**`, `src/__server__/**` | `../ecommerce/src/lib/api/storefront.ts`, `../ecommerce/src/lib/http.ts` | Drop mock adapter, wire `StorefrontApi` wrapper and export helpers for Bonik sections. |
| Storefront config | `src/lib` (new folder), `src/app/layout.tsx` | `../ecommerce/src/lib/storefront-config.ts`, `../ecommerce/src/lib/layouts/homeLayouts.ts` | Port config loader + layout resolver and expose to Bonik pages. |
| Routing scaffold | `src/app/**` | `../ecommerce/src/app/(storefront)/**` | Create `app/(storefront)` tree, move Bonik pages inside and align route defaults. |
| Cart/session | `src/contexts/CartContext.tsx`, `src/app/layout.tsx` | `../ecommerce/src/state/cart-context.tsx`, `../ecommerce/src/app/(storefront)/account/*` | Replace context with persisted cart + reuse auth flows. |
| Styling bridge | `src/theme/**`, `src/components/**` | Bonik theme (current folder) + `../ecommerce/src/lib/utils/format.ts` | Keep styled-components registry, adapt component props to real API responses. |
