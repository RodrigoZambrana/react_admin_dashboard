...
5. Si hay un bloqueo obsoleto, proponer promoción; no iniciarla.

```bash
npm run harness:intake -- --file path/to/intake.json
npm run harness:intake -- promotions
npm run harness:intake -- apply-promotion --id HAR-004 --confirm
```

El pedido extenso de verificación vive en
`harness/tests/fixtures/intake/long-commerce-metrics-request.md`.

`promotions` es read-only. `apply-promotion` sin `--confirm` también. Con
`--confirm` actualiza el backlog canónico y el fragmento dueño; no ejecuta
`lifecycle start`. `--start` está prohibido y falla cerrado.
...
fragmento dueño cuando esa ruta está en el write-set.
