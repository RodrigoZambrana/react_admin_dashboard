...
const SUPERADMIN_NAME = process.env.SEED_SUPERADMIN_NAME ||
...
async function seedSuperAdmin() {
    if (!SUPERADMIN_EMAIL || !SUPERADMIN_PASSWORD) {
        console.log('[seed] Skipping superadmin creation: SEED_SUPERADMIN_EMAIL / SEED_SUPERADMIN_PASSWORD not provided.');
        return null;
    }
    ...
    console.log(`[seed] Superadmin account created for ${normalizedEmail}. (password: ${maskSecret(SUPERADMIN_PASSWORD)})`);
    delete process.env.SEED_SUPERADMIN_PASSWORD;
    return created;
}
async function seedDemoData(superAdminEmail) {
    if (!ENABLE_DEMO_SEED) {
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const expenseStatusId = expenseStatusIds.length > 0
