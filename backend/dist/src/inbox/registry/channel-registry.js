...
let ChannelRegistry = ChannelRegistry_1 = class ChannelRegistry {
    ...
    register(adapter) {
        if (this.adapters.has(adapter.type)) {
            this.logger.warn(`Replacing channel adapter for ${adapter.type}. Check duplicate providers.`);
        }
    ...
    getAdapter(type) {
        ...
        if (!adapter) {
            throw new Error(`No channel adapter registered for ${type}. Ensure providers are configured.`);
        }
        return adapter;
    }
    listAdapters() {
