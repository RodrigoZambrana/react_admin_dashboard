...
const order_finance_service_1 = require("../orders/order-finance.service");
const money_util_1 = require("../common/currency/money.util");
let PaymentsService = class PaymentsService {
    prisma;
    orderFinance;
    constructor(prisma, orderFinance) {
        this.prisma = prisma;
        this.orderFinance = orderFinance;
    }
    async resolvePaymentMethod(client, paymentMethodId, methodName) {
        if (paymentMethodId) {
    ...
    serializePayment(payment) {
        const serializeAttachments = () => payment.attachments?.map((attachment) => ({
            id: attachment.id,
    ...
    extractAttachmentPayload(attachments) {
    ...
    async listPayments(query) {
        ...
        if (query.query) {
            ...
            if (normalized.length) {
                ...
                const orderIdFilter = Number.isFinite(numericOrderId) && String(numericOrderId) === normalized
                    ? {
    ...
    async getPayment(id) {
        ...
        if (!payment) {
            throw new common_1.NotFoundException('accounting.payments.validation.notFound');
        }
    ...
    async createPayment(dto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (!order) {
                throw new common_1.BadRequestException('accounting.payments.validation.orderNotFound');
            }
            const currency = (dto.currency ?? order.orderCurrency ?? 'UYU').toUpperCase();
            const paymentMethod = await this.resolvePaymentMethod(tx, dto.paymentMethodId ?? null, dto.method ?? null);
            const amountDecimal = (0, money_util_1.roundDecimal)(dto.amount, 2);
            ...
            const payment = await tx.payment.create({
                data: {
                    ...
                    currency,
                    type: dto.type ?? client_1.PaymentType.BALANCE,
                    status: dto.status ?? client_1.PaymentStatus.CONFIRMED,
    ...
    async updatePayment(id, dto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (dto.amount !== undefined) {
                data.amount = (0, money_util_1.roundDecimal)(dto.amount, 2).toFixed(2);
            }
            if (dto.currency) {
    ...
    async getAttachment(attachmentId) {
        ...
        return {
            ...
            attachment,
            stream: new common_1.StreamableFile(buffer, {
