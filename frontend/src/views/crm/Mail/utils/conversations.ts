...
const collectParticipants = (mail?: Partial<Mail>) => {
    ...
    ;((mail as { ccAddresses?: string[] })?.ccAddresses ?? []).forEach(push)
    ;((mail as { bccAddresses?: string[] })?.bccAddresses ?? []).forEach(push)
    ;((mail as { replyToAddresses?: string[] })?.replyToAddresses ?? []).forEach(
        push,
    )
