...
const CustomerStatuses = () => {
    ...
    const handleExport = async () => {
        try {
            ...
            const rows = Array.isArray(res.data?.customerStatuses)
                ? res.data.customerStatuses.map((status: any) => ({
                      name: status?.name ?? '',
                      color: status?.color ?? '',
                  }))
                : []
            downloadCsvFile('customer_statuses.csv', rows)
        } catch (error: any) {
            toast.push(
        ...
        <div className="flex flex-col gap-4 h-full">
            <Card className="card-shadow">
                <div className="flex flex-col gap-2 mb-4 md:flex-row md:items-center md:justify-between">
                    <h4>{t('settings.customerStatuses.title')}</h4>
