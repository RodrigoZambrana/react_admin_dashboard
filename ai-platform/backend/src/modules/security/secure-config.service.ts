...
export class SecureConfigService {
  ...
  async getString(key: string): Promise<SecureConfigRecord<string> | null> {
    const record = await this.prisma.secureConfig.findUnique({
      where: { key },
    });
    ...
    return {
      value: this.encryption.decrypt(record.value),
  ...
  async setJson<T>(key: string, payload: T | null): Promise<void> {
    ...
    await this.setString(key, JSON.stringify(payload));
  }
}
