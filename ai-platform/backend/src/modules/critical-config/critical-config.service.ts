...
export class CriticalConfigService {
  ...
  async activateVersion(versionId: string, createdBy?: string) {
    const existing = await this.criticalConfigVersionRepository.findById(versionId);

    if (!existing) {
      throw new NotFoundException(
        `Critical config version ${versionId} was not found`,
      );
    }
    ...
    const key = criticalConfigKeySchema.parse(existing.key);
    const value = parseCriticalConfigValue(key, existing.value);
    const config = await this.criticalConfigVersionRepository.createVersion({
      key,
      value: value as Prisma.InputJsonValue,
      metadata: {
    ...
    this.logger.log(
      JSON.stringify({
        stage: 'critical_config.activated',
        key: config.key,
        version: config.version,
