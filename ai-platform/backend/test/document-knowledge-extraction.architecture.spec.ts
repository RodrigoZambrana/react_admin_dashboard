...
describe('Document knowledge extraction architecture', () => {
  it('keeps the base extractor neutral and routes product/catalog semantics through a dedicated profile boundary', () => {
    ...
    expect(profileConfigSource).toContain('sectionAliasesByAxis');
    ...
    expect(profileResourceSource).not.toContain('"Mercado Pago"');
    expect(profileResourceSource).not.toContain('"Creditel"');
    expect(profileResourceSource).not.toContain('"actualizada"');
    expect(profileResourceSource).not.toContain('"unificada"');
    expect(profileResourceSource).not.toContain('"afirmar"');
    ...
    expect(profileSource).not.toContain("'Mercado Pago'");
    expect(profileSource).not.toContain("'Creditel'");
    expect(profileSource).not.toContain('const productCatalogExtractionCatalog');
    expect(profileSource).not.toContain('const paymentMethodsHeadingTerms');
    expect(profileSource).not.toContain('const quoteFieldsHeadingTerms');
    expect(profileSource).not.toContain('const quoteTransitionHeadingTerms');
    expect(profileSource).not.toContain('const exactHoursSignals');
    expect(profileSource).not.toContain('const exactColorSignals');
