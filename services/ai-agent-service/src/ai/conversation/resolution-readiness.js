...
export const buildResolutionReadiness = ({
  ...
  const normalizedCurrentTurnText = normalizeText(currentTurnText)
  const currentTurnWordCount = normalizedCurrentTurnText
    ? normalizedCurrentTurnText.split(/\s+/u).filter(Boolean).length
    : 0
  const questionLikeTurn = QUESTION_LIKE_REGEX.test(normalizedCurrentTurnText)
  ...
  return {
    ...
    quoteSlotOnlyFollowUp: Boolean(quoteProgression?.slotOnlyFollowUp),
    quoteQuantityOnlyFollowUp: Boolean(quoteProgression?.quantityOnlyFollowUp),
    quoteMeasurementOnlyFollowUp: Boolean(quoteProgression?.measurementOnlyFollowUp),
    quoteAttributeOnlyFollowUp: Boolean(quoteProgression?.attributeOnlyFollowUp),
    quoteAttributeKeys: Array.isArray(quoteProgression?.attributeKeys)
      ? quoteProgression.attributeKeys
      : [],
