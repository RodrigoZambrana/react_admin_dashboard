...
const buildQuoteProgressText = ({
  ...
  if (!topicRecognized) {
    if (measurementLabel) {
      return `Perfecto. Tomo una medida aproximada de ${measurementLabel}. Para avanzar con la cotización, decime también qué producto o servicio buscás.`
    }

    return mode === 'price'
      ? 'Claro. Para orientarte con el precio, decime de qué producto o medida se trata.'
      : 'Claro. Para prepararte un presupuesto, decime qué producto o servicio te interesa y, si aplica, las medidas aproximadas.'
  }
